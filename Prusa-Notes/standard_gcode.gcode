; generated by PrusaSlicer 2.9.4 on 2025-12-04 at 18:49:16 UTC
...
; max_layer_z = 11.80
; objects_info = {"objects":[{"name":"top left.stl","polygon":[[201.005,249.419],[176.505,249.419],[176.505,145.640],[183.576,138.569],[201.005,138.569]]}]}
; filament used [mm] = 3827.29, 0.00, 0.00, 0.00, 0.00
; filament used [g] = 11.42, 0.00, 0.00, 0.00, 0.00
; filament cost = 0.29, 0.00, 0.00, 0.00, 0.00
; filament used [cm3] = 9.21, 0.00, 0.00, 0.00, 0.00
; total filament used for wipe tower [g] = 0.00
; estimated printing time (normal mode) = 27m 2s
; estimated printing time (silent mode) = 30m 46s

; setup print parameters
M73 P0 R27			; set print progress
M73 Q0 S30			; set stealth print progress
M201 X7000 Y7000 Z200 E2500	; set max x, y, z, and extruder acceleration
M203 X400 Y400 Z12 E100		; set max x, y, z, and extruder speeds
M204 P4000 R1200 T5000		; set acceleration baselines for printing (P), retract (R), and travel (T) moves
M205 X8.00 Y8.00 Z2.00 E10.00	; set max x, y, z, and extruder jerk
M205 S0 T0			; set minimum feedrate for printing (S) and set minimum speed for travel (T)
M486 S0				; start an object with the index 0
M486 Atop left.stl		; set the name of the current object
M486 S-1			; start an object with index -1 that can't be canceled/skipped

;TYPE:Custom
M17				; enable all stepper motors
M862.3 P "XL"			; check printer model is Prusa XL
M862.5 P2			; check the gcode level is 2
M862.6 P"Input shaper"		; check the firmware has input shaper feature
M115 U6.2.6+8948		; get firmware version and capabilities and check against minimum firmware version
G90				; set to absolute positioning
M83				; set extruder to relative positioning

M555 X190.805 Y162.069 W10.2 H63.85	; define bounding area of the print
M862.1 T0 P0.4 A0 F0			; check nozzle diameter
...
M217 Z2		; set tool-change z raise

; set temps
M140 S60	; set bed temp and don't wait
G0 Z5		; linear move - Z up
M109 T0 S170	; set extruder temp and wait

; home axes & prepare for mesh bed leveling
G28 X Y		; home XY
G1 F24000	; set move speed
T0 S1 L0 D0	; select tool that will be used to home & MBL (mesh bed leveling)
M84 E		; stop idle hold on extruder (don't lock extruder stepper)
G28 Z		; home z axis
G0 Z5		; linear move - Z up
M104 T0 S70	; set extruder temp and don't wait
M190 S60	; set bed temp and wait
G29 G		; perform MBL (detailed Z-probe)
M109 T0 S170	; set extruder temp and wait

; nozzle cleanup
G1 X222.805 Y155.069 Z5 F24000	; linear move
M302 S155			; set minimum extrusion temp
G1 E-2 F2400			; extruder retraction
M84 E				; stop idle hold on extruder
G29 P9 X190.805 Y155.069 W32 H7	; probe bed to wipe nozzle
G0 Z5 F480			; linear move
M107				; turn off print cooling fan

; MBL
M84 E				; stop idle hold on extruder
G29 P1				; phase 1 of MBL, invalidate the old leveling data to generate new mesh data
G29 P1 X30 Y0 W50 H20 C		; phase 1 of MBL, run bed probing specifying the area on the bed to probe
G29 P3.2			; phase 3 of MBL, fill unpopulated mesh points with fixed value 0.2 mm
G29 P3.13			; phase 3 of MBL, fill unpopulated mesh points with fixed value 0.13 mm
G29 A				; activate unified bed leveling systems
G1 Z10 F720			; linear move - Z up
G1 F24000			; set move speed
P0 S1 L1 D0			; park tool 0

; purge initial tool
M104 T0 S230		; set extruder temp and don't wait
G1 F24000		; set move speed
P0 S1 L2 D0		; park tool 0 with full lift (L2)
M109 T0 S230		; set extruder temp and wait
T0 S1 L0 D0		; select tool (extruder) 0
G92 E0			; set current extruder position to 0
G0 X30 Y-7 Z10 F24000	; linear move
G0 E30 X40 Z0.2 F170	; linear move
M73 P0 R26		; set build progress
G0 X70 E9 F800		; linear move - push filament
G0 X73 Z0.05 F8000	; linear move - Z up
G0 X76 Z0.2 F8000	; linear move - Z up
G1 E-0.8 F2400		; linear move - retract filament
G92 E0			; set current extruder position to 0

; setup more print parameters
G21			; set units to millimeters
G90			; set to absolute positioning
M83			; set extruder to relative positioning
M572 S0.03		; set extruder pressure advance
...
; Filament-specific end gcode
; Filament-specific end gcode
G4				; dwell (pause the machine)
G1 Z16.8			; linear move - Z up
P0 S1				; park tool 0
G1 Z108.8 F300			; linear move - Z up
M73 Q99 S0			; set print progress

; turn off extruder heaters
M104 T0 S0			; set extruder temperature and don't wait
M140 S0				; set bed temperature and don't wait
M107				; turn off print cooling fan
M221 S100			; set extrusion percentage
M84				; stop idle hold on all steppers
M77				; stop print job timer
; max_layer_z = 11.8
M73 P100 R0			; set print progress
M73 Q100 S0			; set stealth print progress

; filament used [mm] = 3827.29, 0.00, 0.00, 0.00, 0.00
; filament used [cm3] = 9.21, 0.00, 0.00, 0.00, 0.00
; filament used [g] = 11.42, 0.00, 0.00, 0.00, 0.00
; filament cost = 0.29, 0.00, 0.00, 0.00, 0.00
; total filament used [g] = 11.42
